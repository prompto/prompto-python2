...
class MemStore(IStore):
    ...
    def doStore(self, storable, withMeta):
        ...
        if previous is None:
            ...
            document.update(previous.document)
        document.update(storable.document)
    ...
    def fetchAllAuditRecords(self, dbId):
        audits = self.fetchAllAuditRecordsStream(dbId)
        return list(audits)

    def fetchAllAuditRecordsAsDocuments(self, dbId):
        audits = self.fetchAllAuditRecordsStream(dbId)
        return list([a.asDocument() for a in audits])

    def fetchAllAuditRecordsStream(self, dbId):
        audits = filter(lambda a: a.instanceDbId == dbId, self.auditRecords.values())
        return sorted(audits, key=lambda a: a.utcTimestamp, reverse=True)

    def fetchDbIdsAffectedByAuditMetadataId(self, dbId):
